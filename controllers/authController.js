...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  if (process.env.NODE_ENV === 'development') signupOptions = req.body;
  const newUser = await User.create(signupOptions);
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('The is no user with this email', 404));
  }
  ...
  const resetURL = `${req.protocol}//${req.get(
    'host'
  )}/aoi/v1/users/resetPassword/${resetToken}`;
  ...
  try {
    ...
    // Reset tokens from database in case error occurs
